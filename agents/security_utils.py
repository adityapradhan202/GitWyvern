...
class SecurityUtils:
    ...
    def vulnerability_count(bandit_cout:str) -> List[int]:
        """Takes in bandit's console output as argument and returns a list of vulnerability count by severity and by confidence."""
    ...
    def vulnerability_count_map(bandit_cout:str) -> Dict[str, Dict[str, int]]:
        """Returns a dictionary for vulnaribility count. Takes bandit console output as argument."""
    ...
    def safety_check(path:str) -> str:
        ...
        bandit_cout = SecurityUtils.bandit_scan(path)
